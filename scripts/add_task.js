...
let globalCategory = 'technical task';
let globalSubtasks = [];
let subtasksList = [];
...
async function addTaskCreateTask() {
	...
	const subtasks = globalSubtasks.map((subtask) => ({
		title: subtask,
		completed: false,
	}));

	const createdTask = {
		board_category: globalBoardCategory.toLowerCase(),
		title: title,
		description: description,
		assignees_ids: contactIds,
		date: date,
		priority: globalPrio.toLowerCase(),
		category: globalCategory.toLowerCase(),
		subtasks: subtasks,
	};
	...
	try {
		...
		if (!response.ok) {
			...
			throw new Error(`Failed adding new task: ${errorData.message || response.statusText}`);
		}
...
function addTaskAssignedToSearch() {
	...
	for (let i = 0; i < contactsToAssigned.length; i++) {
		...
		if (contact.toLowerCase().includes(search)) {
			...
			createContactsContainer.innerHTML += addTaskAssignedToSearchHTML(i, bgColor, contact, assigned);
		}
	}
}
